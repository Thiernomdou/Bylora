    ...
    <nav className="sticky top-0 z-50 bg-[#FAF4EC]/90 backdrop-blur-md border-b border-black/[0.07]">
      <div className="max-w-7xl mx-auto px-6 lg:px-10 py-4 flex items-center justify-between gap-8">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={onOpenAuth}
            className="hidden sm:block text-[14px] font-semibold text-gray-700 hover:text-gray-900 px-4 py-2.5 rounded-full hover:bg-black/[0.05] transition-colors cursor-pointer"
          >
            Se connecter
          </button>
          <button
            onClick={onOpenAuth}
            className="bg-[#FF4D1C] text-white text-[14px] font-bold px-5 py-2.5 rounded-full hover:bg-[#E8421A] transition-colors cursor-pointer shadow-sm"
          >
            Créer un compte
          </button>
        </div>
