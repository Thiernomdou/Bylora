...
type Rating = "connais" | "hesite" | "connais_pas";
type SessionState = "picking" | "question" | "answer" | "done";

const THEME_OPTIONS: { value: ThemeCivique | "all"; label: string; icon: string }[] = [
  { value: "all",          label: "Tous les thèmes",                              icon: "layers"          },
  { value: "republique",   label: THEMES_CIVIQUES.republique.label,   icon: THEMES_CIVIQUES.republique.icon   },
  { value: "institutions", label: THEMES_CIVIQUES.institutions.label, icon: THEMES_CIVIQUES.institutions.icon },
  { value: "droits",       label: THEMES_CIVIQUES.droits.label,       icon: THEMES_CIVIQUES.droits.icon       },
  { value: "histoire",     label: THEMES_CIVIQUES.histoire.label,     icon: THEMES_CIVIQUES.histoire.icon     },
  { value: "societe",      label: THEMES_CIVIQUES.societe.label,      icon: THEMES_CIVIQUES.societe.icon      },
];
...
export default function EntrainementPage() {
  ...
  const [selectedTheme, setSelectedTheme] = useState<ThemeCivique | "all">("all");
  const [queue,         setQueue]         = useState<typeof QUESTIONS_CIVIQUES>([]);
  ...
  const startSession = useCallback((themeOverride?: ThemeCivique | "all") => {
    const theme = themeOverride ?? selectedTheme;
    const pool  = theme === "all"
      ? QUESTIONS_CIVIQUES
      : QUESTIONS_CIVIQUES.filter((q) => q.theme === theme);
  ...
  const rate = useCallback(async (rating: Rating) => {
    ...
    setState("answer");
    if (userId) {
      await supabase.from("civique_user_progress").upsert(
        { user_id: userId, question_id: q.id, parcours, theme: q.theme, rating, updated_at: new Date().toISOString() },
        { onConflict: "user_id,question_id,parcours" }
      );
    }
  }, [queue, current, supabase, userId, parcours]);
      ...
      <div className="px-5 md:px-10 pt-6 pb-6 max-w-5xl mx-auto w-full space-y-5 md:space-y-7">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="text-gray-900 text-[28px] md:text-[36px] font-bold leading-tight">S&apos;entraîner</h1>
          </div>
        </div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-gray-400 text-[12px] mt-0.5">
                  {t.value === "all"
                    ? `${QUESTIONS_CIVIQUES.length} questions`
                    : `${QUESTIONS_CIVIQUES.filter((q) => q.theme === t.value).length} questions`}
                </p>
              </div>
  ...
  const q               = queue[current];
  const theme           = THEMES_CIVIQUES[q.theme];
        ...
        <div className="flex flex-col items-stretch rounded-2xl border border-black/[0.07] bg-white shadow-sm overflow-hidden min-h-[240px] md:min-h-[400px]">
          ...
          <div className="px-4 md:px-5 pt-3 md:pt-4 pb-0">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border" style={{ background: `${theme.accent}15`, borderColor: `${theme.accent}30` }}>
              <span className="material-symbols-outlined" style={{ fontSize: "13px", color: theme.accent }}>{theme.icon}</span>
              <span className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: theme.accent }}>{theme.label}</span>
            </div>
          </div>
      ...
      <div className="flex flex-col items-stretch rounded-2xl overflow-hidden border border-black/[0.07] bg-white shadow-sm">
        ...
        <div className="flex w-full flex-col items-start gap-3 md:gap-4 p-4 md:p-7">
          ...
          <div>
            ...
            <p className="text-gray-600 text-[13px] md:text-[15px] leading-relaxed">{q.options[q.correct]}</p>
            {q.explication && (
              <p className="text-gray-400 text-[12px] md:text-[13px] leading-relaxed mt-2">{q.explication}</p>
            )}
          </div>
        </div>
      </div>
